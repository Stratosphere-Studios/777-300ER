...
fmsPages["EFISCTL152"].getPage=function(self,pgNo,fmsID)
    ...
    local mda=string.format("%04d",B777DR_efis_baro_alt_ref_capt)
    if fmsID=="fmsR" then 
        ...
        mda=string.format("%04d",B777DR_efis_baro_alt_ref_fo)
    elseif  B777DR_efis_baro_ref_capt_sel_dial_pos ==1 then 
        baro=baro*33.863892  
        baroString=" "..string.format("%04d",baro) 
    else
        baroString=string.format("%02.2f",baro) 
...
fmsPages["EFISCTL152"].getSmallPage=function(self,pgNo,fmsID)
    local mode="IN "
    local baromode=B777DR_efis_baro_ref_capt_sel_dial_pos
    local range=ranges[simDR_range_dial_capt+1]
    if fmsID=="fmsR" then 
        baromode=B777DR_efis_baro_ref_fo_sel_dial_pos 
...
fmsPages["EFISOPTIONS152"].getPage=function(self,pgNo,fmsID)
    ...
    if B777DR_nd_wxr==1 then wxrSelected="<SEL>" end
    if fmsID=="fmsR" then 
        if B777DR_nd_fo_ftc==1 then tfcSelected="<SEL>" end
        if B777DR_nd_fo_vor_ndb==1 then staSelected="<SEL>" end
        if B777DR_nd_fo_terr==1 then terrSelected="<SEL>" end
        if B777DR_nd_fo_wpt==1 then wptSelected="<SEL>" end
        if B777DR_nd_fo_apt==1 then arptSelected="<SEL>" end
        ...
        if B777DR_nd_capt_ftc==1 then tfcSelected="<SEL>" end
        if B777DR_nd_capt_vor_ndb==1 then staSelected="<SEL>" end
        if B777DR_nd_capt_terr==1 then terrSelected="<SEL>" end
        if B777DR_nd_capt_wpt==1 then wptSelected="<SEL>" end
        if B777DR_nd_capt_apt==1 then arptSelected="<SEL>" end
