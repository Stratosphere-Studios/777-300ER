--[[
****************************************************************************************
* Program Script Name	:	B777.42.xt.EEC.GE.lua
...
function throttle_resolver_angle_GE(engine_in)
  ...
  thrust_ratio_factor = B777DR_display_N1_max[engine_in] / 116.0  --117.5
  ...
  local N1_target=B777_rescale(0.0, 0.75, 0.88, 2.05, throttle_angle)
...
function engine_idle_control_GE(altitude_ft_in)
  ...
  if simDR_onGround == 1 then
    if simDR_temperature < 15.0 then
      simDR_engine_high_idle_ratio = B777_rescale(-75.0, 1.04, 14.99, 1.249, simDR_temperature)
    else
      simDR_engine_high_idle_ratio = B777_rescale(15.0, 1.25, 75.0, 1.49, simDR_temperature)
    end
  end
...
function in_flight_N1_GE(altitude_ft_in, delta_t_isa_K_in)
    ...
    if string.match(simConfigData["data"].PLANE.engines, "B1F") then
      ...
      elseif simDR_altitude <= 30000 then
        --climb_rate_fpm = 2500
        climb_rate_fpm = B777_rescale(10000.0, 2500.0, 20000.0, 2250.0, simDR_altitude)
      elseif simDR_altitude <= 40000 then
        --climb_rate_fpm = 2000
        climb_rate_fpm = B777_rescale(10000.0, 2250.0, 20000.0, 1500.0, simDR_altitude)
...
function N1_display_GE(altitude_ft_in, thrust_N_in, n1_factor_in, engine_in)
    ...
    * corrected_thrust_lbf^2 + (-0.0027 * mach^2 + 0.0011 * mach + 0.0036) * corrected_thrust_lbf + (36.93 * mach + 20.96)
    ...
    if (string.match(B777DR_ref_thr_limit_mode, "TO") or (simDR_onGround == 1 and B777DR_ref_thr_limit_mode == "GA"))
      or ((B777DR_ref_thr_limit_mode == "NONE" or B777DR_ref_thr_limit_mode == "") and B777DR_radio_altitude < 2000) then
...
function N2_display_GE(engine_N1_in, engine_in)
  ...
  elseif N2_display < 48.0 then
    N2_display = B777_rescale(35.0, 35.0, 65.0, 48.0, simDR_N2[engine_in])
  end]]--
...
function EGT_display_GE(engine_in)
  ...
  elseif simDR_engn_EGT_c[engine_in] < 375 then
    ...
    EGT_display = B777_rescale(0.0, 0.0, 985.0, 985.0, simDR_engn_EGT_c[engine_in])
  end
...
function GE(altitude_ft_in)
  ...
  if simDR_onGround == 1 or takeoff_thrust_n1==0.0 or B777DR_ref_thr_limit_mode == "" or B777DR_ref_thr_limit_mode == "NONE" then
    ...
    takeoff_thrust_n1_throttle = B777_rescale(0.0, 0.0, tonumber(takeoff_TOGA_n1), 1.0, tonumber(takeoff_thrust_n1))
    ...
    for i = 0, 3 do
      ...
      display_N1_max[i] = math.min(string.format("%4.1f",takeoff_thrust_n1) + packs_adjustment_value + engine_anti_ice_adjustment_value, 117.5)
		...
		B777DR_TO_throttle = takeoff_thrust_n1_throttle
    --print("did init")
    --simCMD_pause:once() 
  end
  ...
  elseif string.match(B777DR_ref_thr_limit_mode, "CLB") then
    _, N1_actual, _, _, N1_real_max_climb, _, _, _ = in_flight_N1_GE(altitude_ft_in, 0)
      ...
      if B777DR_display_N1_max[i] < B777DR_display_N1_ref[i] then
        display_N1_max[i] = B777DR_display_N1_ref[i]
      else
  ...
  elseif B777DR_ref_thr_limit_mode == "GA" or (simDR_onGround == 0 and B777DR_ref_thr_limit_mode == "" or B777DR_ref_thr_limit_mode == "NONE") then
    ...
    if math.max(simDR_engn_thro_use[0], simDR_engn_thro_use[1], simDR_engn_thro_use[2], simDR_engn_thro_use[3]) == 1
      ...
      and (B777DR_display_N1[0] < simDR_N1_target_bug[0] or B777DR_display_N1[1] < simDR_N1_target_bug[1]
          or B777DR_display_N1[2] < simDR_N1_target_bug[2] or B777DR_display_N1[3] < simDR_N1_target_bug[3]) then
  ...
  for i = 0, 3 do
    ...
    B777DR_display_N1_ref[i] = display_N1_ref[i]
    B777DR_display_N1_max[i] = display_N1_max[i]
  end
	for i = 0, 3 do
    ...
    B777DR_display_GE_EGT[i] = math.max(EGT_display[i], 0.0)

    B777DR_throttle_resolver_angle[i] = throttle_resolver_angle_GE(i)
	end
