...
fmsPages["PROGRESS"].getPage=function(self,pgNo,fmsID)
  ...
  if page=="1" then
	...
	if B777BR_tod>0 and B777BR_totalDistance>B777BR_tod then
	  local dist=B777BR_totalDistance-B777BR_tod
	  l10=" TOP OF DES             "
...
fmsPages["PROGRESS"].getPage=function(self,pgNo,fmsID)
  ...
  if pgNo==1 then 
    ...
    local distToNext=tonumber(string.sub(B777DR_ND_waypoint_distance,1,5))
    --print("B777DR_ND_waypoint_distance "..string.sub(B777DR_ND_waypoint_distance,1,5))
    local time_to_waypoint=0
    if B777DR_ND_current_waypoint~="-----" and distToNext~=nil then
    ...
    --local distToNext2=tonumber(string.sub(B777DR_ND_waypoint_distance,1,5))
    if B777DR_next_waypoint~="-----" and B777DR_next_waypoint_dist>0 and B777DR_ND_current_waypoint~="-----" and distToNext~=nil then
      --print("B777DR_next_waypoint_dist "..B777DR_next_waypoint_dist)
      time_to_waypoint = time_to_waypoint+((B777DR_next_waypoint_dist / math.max(default_speed, actual_speed)) * 3600)
      dtgNext=string.format("%4d",distToNext+B777DR_next_waypoint_dist)
      f_used_toNext=time_to_waypoint*ffkgs
    ...
    local destText=" --- ----   --.-"
    if B777BR_totalDistance>0 then
      local dist=B777BR_totalDistance
      local hours = 0
      local mins = 0
      local secs = 0
    ...
    "                        ",
    "                        ",
    string.format("%-5s",B777DR_ND_current_waypoint).."   "..dtgTO.." ".. string.sub(B777DR_ND_waypoint_eta,1,4).."  "..toFuel ,
    "                        ",
    string.format("%-5s", B777DR_next_waypoint).."   "..dtgNext.." ".. string.sub(B777DR_next_waypoint_eta,1,4).."  "..nextFuel,
    "                        ",
    B777DR_destination.."    "..destText,
    "                        ",
    selSpd.."      "..todText, 
    "------------------------",
    "<POS REPORT     POS REF>"
  ...
  elseif pgNo==2 then
    ...
    local fuel1=string.format("%4.1f",(B777DR_engine_used_fuel[0]*mult/1000))
    local fuel2=string.format("%4.1f",(B777DR_engine_used_fuel[1]*mult/1000))
    local fuel3=string.format("%4.1f",(B777DR_engine_used_fuel[2]*mult/1000))
    local fuel4=string.format("%4.1f",(B777DR_engine_used_fuel[3]*mult/1000))
    local fuel_remaining=string.format("%5.1f",(simDR_fueL_tank_weight_total_kg*mult/1000))
    local windSide="L"
    local latWind="  0"
    local longWind="  0"
    if B777DR_ND_Wind_Bearing<-90 then 
      windSide="R"
      local angle=math.rad(180-(B777DR_ND_Wind_Bearing*-1))
      latWind=string.format("%3d",math.sin(angle)*simDR_wind_speed)
      longWind=string.format("%3d",math.cos(angle)*simDR_wind_speed)
    elseif B777DR_ND_Wind_Bearing<0 then 
      windSide="R"
      local angle=math.rad(B777DR_ND_Wind_Bearing*-1)
      latWind=string.format("%3d",math.sin(angle)*simDR_wind_speed)
      longWind=string.format("%3d",math.cos(angle)*simDR_wind_speed)
    elseif B777DR_ND_Wind_Bearing>90 then
      windSide="L"
      local angle=math.rad(180-B777DR_ND_Wind_Bearing)
      latWind=string.format("%3d",math.sin(angle)*simDR_wind_speed)
      longWind=string.format("%3d",math.cos(angle)*simDR_wind_speed)
    else --0 to 90
      windSide="L"
      local angle=math.rad(B777DR_ND_Wind_Bearing)
      latWind=string.format("%3d",math.sin(angle)*simDR_wind_speed)
      longWind=string.format("%3d",math.cos(angle)*simDR_wind_speed)
    end
    lastPage[fmsID]= {
    "        PROGRESS        ",
    "                        ",
    longWind.."KT "..string.format("%10s",B777DR_ND_Wind_Line.."KT").."  "..windSide..latWind .."KT",
    "                        ",
    "L *.*              +**FT",
    "                        ",
    string.format("%03.0f",B777DR_TAS_pilot).."                ".. string.format("%3d",simDR_air_temp) .."`C",
    "                        ",
...
fmsPages["PROGRESS"].getSmallPage=function(self,pgNo,fmsID)
  ...
  if pgNo==1 then 
    local fuel_ata=string.format("%5.1f",(B777DR_last_waypoint_fuel*mult/1000))
    return {
"                    1/3 ",
" LAST   ALT   ATA   FUEL",
string.format("%-5s", B777DR_last_waypoint).."        "..string.sub(B777DR_waypoint_ata,1,4).."z "..fuel_ata ,
" TO      DTG  ETA       ",
"                 Z      ",
" NEXT                   ",
"                 Z      ",
...
elseif pgNo==2 then
  ...
  local fuel_used=string.format("%5.1f",(B777DR_engine_used_fuel[4]*mult/1000))
  local tWind=" H/WIND"
  if B777DR_ND_Wind_Bearing>-90 and B777DR_ND_Wind_Bearing<90 then tWind=" T/WIND" end
  return {
    "                    2/3 ",
    tWind.."   WIND    X/WIND",
    "                        ",
