...
load_past = createGlobalPropertyfa("Strato/777/hydraulics/load_past", {0, 0, 0})
...
hyd_qty_initial = createGlobalPropertyfa("Strato/777/hydraulics/qty_initial", {0.98, 0.95, 0.97})
	...
	if tmp + delay <= Round(sasl.getElapsedSeconds(timer), 1) then
		for i=1,4 do
			...
			if pumps_on > 0 and round(get(t_phi) / 180) % 2 == 0 then
				...
				local load_past = globalPropertyfae("Strato/777/hydraulics/load_past", sys_idx)
				local decrease = 0 --decrease due to performance degradation when overheating and due to change in load
				if get(load_total) > get(load_past) then
					decrease = 100 * Round(get(load_total) - get(load_past), 1)
				end
				if i ~= 2 then
					set(load_past, get(load_total))
				end
...
function UpdateQuantity()
	for i=1,4 do
		...
		local qty_initial = globalPropertyfae("Strato/777/hydraulics/qty_initial", sys_idx)
		if math.abs(get(p_temp) - get(oat)) >= 20 then --the higher the temperature, the higher the oil density => qty will increase with temperature
			local increase =  math.floor((get(p_temp) - get(oat)) / 20) * 0.01
			if get(qty) ~= get(qty_initial) + increase then
				set(qty, get(qty_initial) + increase)
			end
		elseif math.abs(get(d_temp) - get(oat)) >= 20 then
			local increase =  math.floor((get(d_temp) - get(oat)) / 20) * 0.01
			if get(qty) ~= get(qty_initial) + increase then
				set(qty, get(qty_initial) + increase)
			end
		end
