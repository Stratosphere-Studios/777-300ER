...
--   fmsModules["fmsR"].notify="ENTER IRS POSITION"
  B777DR_fmc_notifications[12]=1
end
doIRSAlign={}
doIRSAlign["irsL"]=function()
  if irsSystem["irsL"].align(irsSystem["irsL"])==true then B777DR_iru_status[0]=2 end
...
doIRSAlign["irsC"]=function()
  if irsSystem["irsC"].align(irsSystem["irsC"])==true then B777DR_iru_status[1]=2 end
  if irsSystem["setPos"]==false and irsSystem["irsL"]["aligned"]==true and irsSystem["irsC"]["aligned"]==true and irsSystem["irsR"]["aligned"]==true then 
     irs_notify()
  end 
end
doIRSAlign["irsR"]=function()
  if irsSystem["irsR"].align(irsSystem["irsR"])==true then B777DR_iru_status[2]=2 end
  if irsSystem["setPos"]==false and irsSystem["irsL"]["aligned"]==true and irsSystem["irsC"]["aligned"]==true and irsSystem["irsR"]["aligned"]==true then 
     irs_notify()
  end 
end
...
irsSystem.update=function()
  --Marauder28
  --GPS/IRS DISPLAY
  B777DR_ND_GPS_Line = "GPS"
  if B777DR_iru_status[0]==0 then irsSystem["irsL"]["aligned"] = false end
  if B777DR_iru_status[1]==0 then irsSystem["irsC"]["aligned"] = false end
  if B777DR_iru_status[2]==0 then irsSystem["irsR"]["aligned"] = false end
  if irsSystem["irsL"]["aligned"] == true then
	B777DR_ND_IRS_Line = "IRS (L)"
  elseif irsSystem["irsC"]["aligned"] == true then
	B777DR_ND_IRS_Line = "IRS (C)"
  elseif irsSystem["irsR"]["aligned"] == true then
	B777DR_ND_IRS_Line = "IRS (R)"
  end

  --Update Set Hdg on FMC
  if (B777DR_iru_mode_sel_pos[0] == 3 or B777DR_iru_mode_sel_pos[1] == 3 or B777DR_iru_mode_sel_pos[2] == 3) then
  ...
  if irsSystem["irsL"]["aligned"] == true and irsSystem["irsC"]["aligned"] == true and irsSystem["irsR"]["aligned"] == true then
	B777DR_ND_IRS_Line = "IRS (3)"
  end
    
    if irsSystem["setPos"]==true and irsSystem[irsFromNum(B777DR_irs_src_capt)]["aligned"]==true then B777DR_pfd_mode_capt=1 else B777DR_pfd_mode_capt=0 end
    if irsSystem["setPos"]==true and irsSystem[irsFromNum(B777DR_irs_src_fo)]["aligned"]==true then B777DR_pfd_mode_fo=1 else B777DR_pfd_mode_fo=0 end
    
    if irsSystem[irsFromNum(B777DR_irs_src_capt)]["aligned"]==false or irsSystem[irsFromNum(B777DR_irs_src_fo)]["aligned"]==false or irsSystem["setPos"]==false then
      irsSystem.setStatus()
    end
    if B777DR_iru_mode_sel_pos[0]==0 then B777DR_iru_status[0]=0 cancelAlign(doIRSAlign["irsL"])
    elseif B777DR_iru_status[0]==1 and B777DR_iru_mode_sel_pos[0]==2 then B777DR_iru_status[0]=4 irsSystem.align("irsL",false)
    elseif B777DR_iru_status[0]==1 then
      irsSystem["irsL"]["aligned"]=false
      cancelAlign(doIRSAlign["irsL"])
    end
    if B777DR_iru_mode_sel_pos[1]==0 then B777DR_iru_status[1]=0 cancelAlign(doIRSAlign["irsC"])
    elseif B777DR_iru_status[1]==1 and B777DR_iru_mode_sel_pos[1]==2 then B777DR_iru_status[1]=4 irsSystem.align("irsC",false) 
    elseif B777DR_iru_status[1]==1 then
      irsSystem["irsC"]["aligned"]=false
      cancelAlign(doIRSAlign["irsC"])
    end
    if B777DR_iru_mode_sel_pos[2]==0 then B777DR_iru_status[2]=0 cancelAlign(doIRSAlign["irsR"])
    elseif B777DR_iru_status[2]==1 and B777DR_iru_mode_sel_pos[2]==2 then B777DR_iru_status[2]=4 irsSystem.align("irsR",false) 
    elseif B777DR_iru_status[2]==1 then
      irsSystem["irsR"]["aligned"]=false
      cancelAlign(doIRSAlign["irsR"])
    end
  if  B777DR_iru_status[0]==4 then B777DR_CAS_memo_status[14]=1 else B777DR_CAS_memo_status[14]=0 end
  if  B777DR_iru_status[1]==4 then B777DR_CAS_memo_status[15]=1 else B777DR_CAS_memo_status[15]=0 end
  if  B777DR_iru_status[2]==4 then B777DR_CAS_memo_status[16]=1 else B777DR_CAS_memo_status[16]=0 end 
  ...
  if irsSystem["motion"]["irsL"]==true or irsSystem["motion"]["irsC"]==true or irsSystem["motion"]["irsR"]==true then 
    B777DR_CAS_advisory_status[233] = 1   
  else
    B777DR_CAS_advisory_status[233] = 0
  end
    
  
  difLat=simDR_latitude-startLat
  difLon=simDR_longitude-startLon
  if irsSystem["irsL"]["aligned"]==false and B777DR_iru_mode_sel_pos[0]==2 and (difLat>0.0001 or difLat<-0.0001 or difLon>0.0001 or difLon<-0.0001) then
    irsSystem["motion"]["irsL"]=true
  end 
  if irsSystem["irsC"]["aligned"]==false and B777DR_iru_mode_sel_pos[1]==2 and (difLat>0.0001 or difLat<-0.0001 or difLon>0.0001 or difLon<-0.0001) then
    irsSystem["motion"]["irsC"]=true
  end  
  if irsSystem["irsR"]["aligned"]==false and B777DR_iru_mode_sel_pos[2]==2 and (difLat>0.0001 or difLat<-0.0001 or difLon>0.0001 or difLon<-0.0001) then
...
irsSystem.getStatus=function(systemID)
  ...
  if B777DR_iru_status[irsSystem[systemID]["index"]]==0 then return systemID:sub(4, 4).." OFF" end
  if B777DR_iru_status[irsSystem[systemID]["index"]]==1 then return systemID:sub(4, 4).." ALIGN" end
  if B777DR_iru_status[irsSystem[systemID]["index"]]==2 then return systemID:sub(4, 4).." ALIGNED" end
  if B777DR_iru_status[irsSystem[systemID]["index"]]==4 then return systemID:sub(4, 4).. timeToAlignString(irsSystem[systemID]["alignStarted"]) .. " MIN" end
end
