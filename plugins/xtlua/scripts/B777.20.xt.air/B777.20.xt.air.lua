...
function Consumer:new(consumptio, opendelay, closedelay, sourcelistener, automanager)
    local t = {consumption = consumptio, openDelay = opendelay, closeDelay = closedelay, sourceListener = sourcelistener, autoManager = automanager}
    setmetatable(t, self)
    self.__index = self
    return t
end

local packL = Consumer:new(6, 5, 5, leftSourceListener,
function(self)
...
local packR = Consumer:new(6, 5, 5, rightSourceListener,
function(self)
    if self.switchState == 1 and self.valveState == 0 then
        run_after_time(function() self.valveState = 1; self:sourceListener() end, self.openDelay)
    end
...
local trimL = Consumer:new(1, 1, 1, leftSourceListener,
function (self)
...
local trimR = Consumer:new(1, 1, 1, rightSourceListener,
function (self)
    if self.switchState == 1 then
...
local eng1Starter = Consumer:new(7, 0, 0)
local eng2Starter = Consumer:new(11, 0, 0)
